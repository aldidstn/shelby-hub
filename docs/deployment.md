...
Disable new uploads and purchases at the deployment layer, retain the database and every referenced `PREMIUM_MASTER_KEY_V*` secret, and deploy the prior application version. Do not delete V2 records or wrapped-key ciphertext. Registry events are replayable by resetting only the `registry_v2_event_version` row in `indexer_state`.
