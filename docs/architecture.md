...
For connected wallets, changing the workspace dropdown requests the same network in the wallet before persisting the selection. Reconnecting a wallet also reconciles it with the saved selection. Wallet authentication challenges use the selected CAIP-2 chain ID (`aptos:testnet` or `aptos:shelbynet`), and the server stores that chain ID with the single-use nonce so verification cannot be replayed against a different network.
